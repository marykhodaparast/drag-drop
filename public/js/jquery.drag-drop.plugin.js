(function ($) {
    ...
    function cancelDestElement(options) {
        if ($destElement != null) {
            if (options.dropClass) $destElement.removeClass(options.dropClass);
            $destElement = null;
        }
        ...
        onStart: function (event) {
            ...
            if ($element) {
                ...
                if (event.type == "touchstart") {
                    dragOffsetX =
                        event.originalEvent.touches[0].clientX - offset.left;
                    dragOffsetY =
                        event.originalEvent.touches[0].clientY - offset.top;
                } else {
                ...
                $(window)
                    .bind(
                        "mousemove.dragdrop touchmove.dragdrop",
                        { source: $me },
                        methods.onMove
                    )
                    .bind(
                        "mouseup.dragdrop touchend.dragdrop",
                        { source: $me },
                        methods.onEnd
                    );
